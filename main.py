...
from src.operations import search_operations_by_description, search_operations_by_currency
from src.operations import search_operations_by_word, count_categories
...
Доступные для фильтровки статусы: EXECUTED, CANCELED, PENDING''', end="\n\n")
    ...
    print(f"Отсортировать операции по дате? Да/Нет", end="\n\n")
    user_sort_by_date = input()
    print(end="\n\n")
    if user_sort_by_date.lower() == 'да':
        print(f"Отсортировать по возрастанию или по убыванию?", end="\n\n")
        user_sort = input()
        print(end="\n\n")
    ...
    print(f"Выводить только рублевые транзакции? Да/Нет", end="\n\n")
    ...
    print(f"Отфильтровать список транзакций по определенному слову в описании? Да/Нет", end="\n\n")
    user_answer = input()
    print(end="\n\n")
    if user_answer.lower() == 'да':
        print(f"Введите слово, пример: 'Перевод', 'счет', 'карты', 'карту' и т.д.", end="\n\n")
        user_filtr_word = input()
        print(end="\n\n")
        transactions_by_state = search_operations_by_word(transactions_by_state, user_filtr_word)


    print(f"Распечатываю итоговый список транзакций...", end="\n\n")
